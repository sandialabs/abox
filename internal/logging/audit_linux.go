...
// Events are written to syslog and are readable via `journalctl -t abox`. Returns
// nil if syslog is unavailable.
func newAuditHandler() slog.Handler {
	return newSyslogHandler()
}

// initAuditSink is a no-op on Linux: syslog requires no directory setup.
func initAuditSink() {}

// closeAuditSink is a no-op on Linux: the syslog writer is managed by newSyslogHandler.
func closeAuditSink() {}
