...
// testSockPath is a stand-in --socket path used across the BuildArgs tests.
const testSockPath = "/tmp/vmnet.sock"
...
func TestBuildArgs_FixedOrder(t *testing.T) {
	// --socket must precede --operation-mode must precede conditional flags.
	cfg := HelperConfig{
	...
	got := BuildArgs(cfg)
	sockIdx := indexOf(got, "--socket="+testSockPath)
	modeIdx := indexOf(got, "--operation-mode=host")
	isoIdx := indexOf(got, "--enable-isolation")
	idIdx := indexOf(got, "--interface-id=x")
	if sockIdx >= modeIdx || modeIdx >= isoIdx || isoIdx >= idIdx {
		t.Errorf("flags out of order: socket=%d mode=%d iso=%d id=%d in %v",
			sockIdx, modeIdx, isoIdx, idIdx, got)
	}
}
