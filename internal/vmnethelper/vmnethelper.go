...
type HelperConfig struct {
	...
	SocketPath      string // --socket path: the helper binds a unix datagram socket here and waits for the VMM to connect (no fd is passed through sudo)
...
//
// --socket (not --fd) is used deliberately: the helper binds the datagram
// socket itself and the VMM connects to it, so no file descriptor has to be
// inherited across the sudo boundary (sudo's closefrom would strip it).
...
func ResolveBinaryPath() (string, error) {
	...
	return "", errors.New(
		"vmnet-helper not found — install with " +
			...
			"(brew trust needed on Homebrew 6.0.0+) or " +
