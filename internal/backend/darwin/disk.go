...
// If the image is only in user cache, it copies it to the backend-accessible location.
func (m *DiskManager) EnsureBaseImage(ctx context.Context, client rpc.PrivilegeClient, inst *config.Instance, paths *config.Paths) error {
	...
	backendBaseImage := filepath.Join(paths.BaseImages, inst.Base+".qcow2")

	// Check if base image exists in either location
	_, userErr := os.Stat(userBaseImage)
	_, backendErr := os.Stat(backendBaseImage)

	if userErr != nil && backendErr != nil {
	...
	// If already in backend location, nothing to do
	if backendErr == nil {
		return nil
	}

	// Copy from user cache to backend location
	logging.Debug("copying base image to backend storage directory")
