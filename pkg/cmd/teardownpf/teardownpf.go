...
func run(opts *Options) error {
	...
	// Probe before the RPC so we can tell the user whether anything actually
	// changed (the helper silently no-ops when no abox lines are present).
	hadRefs, _ := privilege.HasAnchorReferences(privilege.PfconfDefaultPath)

	priv, err := f.PfClientFor("")
	if err != nil {
		return fmt.Errorf("failed to start privilege helper: %w", err)
	}

	pf := firewall.NewPfClient(priv)
	if err := pf.TeardownConfig(context.Background()); err != nil {
		return err
	}
