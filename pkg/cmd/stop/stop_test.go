...
func TestNewCmdStop_FlagParsing(t *testing.T) {
	...
	var gotOpts *Options
	cmd := NewCmdStop(f, func(o *Options) error {
		gotOpts = o
		return nil
	})
...
func TestNewCmdStop_RequiresName(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &factory.Factory{IO: ios, ColorScheme: cmdutil.NewColorScheme(false)}
...
func TestNewCmdStop_MultipleNames(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := &factory.Factory{IO: ios, ColorScheme: cmdutil.NewColorScheme(false)}
	...
	cmd.SetArgs([]string{"vm1", "vm2", "vm3"})

	if err := cmd.Execute(); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(gotOpts.Names) != 3 {
		t.Errorf("Names = %v, want 3 names", gotOpts.Names)
	}
}
