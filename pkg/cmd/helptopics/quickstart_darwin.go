...
PREREQUISITES
  ...
  - vmnet-helper installed via Homebrew (required; installs off PATH)
      ...
      brew trust nirs/vmnet-helper   # Homebrew 6.0.0+ only: trust third-party taps
...
MACOS-SPECIFIC NOTES
  - 'abox mount'/'abox unmount' (SSHFS) need a FUSE + sshfs install. Prefer the
    kext-less fuse-t (no System Extension approval or reboot, unlike macFUSE):
    'brew tap macos-fuse-t/homebrew-cask' then 'brew install fuse-t fuse-t-sshfs'.
    Or use 'abox scp' to move files between the host and the VM.
