...
// tcpdumpInstallHint is appended to the Long help text on macOS. tcpdump ships
// with macOS, so no package installation is needed.
...
// tcpdumpNotFoundHint is shown when tcpdump is not on PATH on macOS.
const tcpdumpNotFoundHint = "tcpdump ships with macOS; check that /usr/sbin is on your PATH"

// resolveCaptureInterface returns the real vmnet bridge interface name
// (e.g. "bridge100") that the vfkit backend persisted into
// BackendConfig["bridge"] after the VM booted. The logical resource name in
// inst.Bridge (e.g. "abox-dev") is not a host interface on macOS and would fail
// with "no such device" if passed to tcpdump directly.
func resolveCaptureInterface(inst *config.Instance) (string, error) {
	...
	s, ok := inst.BackendString(config.BackendKeyBridge)
	if !ok {
...
// hintForTcpdumpError returns a remediation hint for common tcpdump failures on
// macOS.
